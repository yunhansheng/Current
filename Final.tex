...
\begin{defn}
A \textbf{model category} if a category $\mathcal{C}$ with three subclasses of morphisms specified: the weak equivalences $\W$, the fibrations $\Fib$ and the cofibrations $\Cof$. They satisfy the following axioms:\begin{enumerate}[(i)]
    ...
    \item Any two classes determine the third as follows:\begin{enumerate}[(a)]
        \item $W=\Fib^r\circ\Cof^l$,
        \item $\Fib=(\W\cap\Cof)^r$
        \item $\Cof=(\W\cap\Fib)^l$
    \end{enumerate}
\end{enumerate}
\end{defn}
